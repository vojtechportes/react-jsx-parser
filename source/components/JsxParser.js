...
export default class JsxParser extends Component {
  ...
  parseExpression = (expression) => {
    switch (expression.type) {
      ...
      case "CallExpression":
        ...
        if (typeof this.props.bindings[expression.callee.name] === "function") {
          const parsedArgs = expression.arguments.map(this.parseExpression);

          return this.props.bindings[expression.callee.name](...parsedArgs);
        } else {
      ...
      case "ObjectExpression":
        ...
        return object;
      case "BlockStatement": {
        return expression.body.map(this.parseExpression);
      }
      case "ReturnStatement": {
        return this.parseExpression(expression.argument);
      }
      case "VariableDeclaration": {
        return expression.declarations.map(this.parseExpression);
      }
      case "VariableDeclarator": {
        this.props.bindings[expression.id.name] = this.parseExpression(
          expression.init
        );
      ...
      case "AssignmentExpression": {
        this.props.bindings[expression.left.name] = expression.right.value;
      ...
      case "IfStatement":
        if (this.parseExpression(expression.test)) {
          return this.parseExpression(expression.consequent);
        } else {
          if (expression.alternate === null) {
            return undefined;
          } else {
            return this.parseExpression(expression.alternate);
          }
        }
  ...
  parseElement = (element) => {
    ...
    if (/^(html|head|body)$/i.test(name))
      return childNodes.map((c) => this.parseElement(c));
    ...
    if (!resolvePath(components, name)) {
      if (componentsOnly) {
        onError(
          new Error(
            `The componenet <${name}> is unrecognized, and will not be rendered.`
          )
        );
        return undefined;
      }
